...
index_of_string_in_array=0
echo "${input_array[@]}" | make run-martynov.alexey/1 VALGRIND="--leak-check=full --track-origins=yes --xml=yes --xml-file='memcheckRes_1_3_1.xml'" ARGS="3" > $TEMP_FILE_WITH_OUTPUT_STRINGS 
while read line ; do
