...
echo "${input_array[@]}" | make run-martynov.alexey/1 VALGRIND="--leak-check=full --track-origins=yes --xml=yes --xml-file='memcheckRes_1_4.xml'" ARGS="4 ascending 5" > $TEMP_FILE_WITH_OUTPUT_STRINGS 
while read line ; do
