...
trap cleanup EXIT
echo "Test case in which input array ends with 0 after 2"
RANGE=20
...
input_array[$MAX_NUM_OF_EL_IN_ARRAY]=2
...
index_of_string_in_array=0
echo "${input_array[@]}" | make run-martynov.alexey/1 VALGRIND="--leak-check=full --track-origins=yes --xml=yes --xml-file='memcheckRes_1_3_2.xml'" ARGS="3" > $TEMP_FILE_WITH_OUTPUT_STRINGS 
while read line ; do
