...
sed -e 's/</&lt/g' tests.out > tests.out
sed -e 's/>/&gt/g' tests.out > tests.out
awk 'BEGIN{print "<table BORDER=0 CELLSPACING=2 CELLPADDING=2> <caption> <b> <br> Memcheck tool: <br> </b> </caption>"} {print "<tr>";print "<td>" $0"</td>";print "</tr>"} END{print "</table>"}' < tests.out >> $WORKSPACE/results.html
