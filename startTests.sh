...
valgrind --tool=cachegrind $WORKSPACE/run_tests.sh 2>&1 >/dev/null | awk 'BEGIN{print "<table BORDER=0 CELLSPACING=2 CELLPADDING=2> <caption> <b> <br> Cachegrind results: <br> </b> </caption>"} {print "<tr>";print "<td>" $0"</td>";print "</tr>"} END{print "</table>"}' >> $WORKSPACE/results.html
